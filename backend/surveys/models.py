...
from surveys.constants import (
    QUESTION_TYPE_SINGLE, QUESTION_TYPE_MULTIPLE, QUESTION_TYPE_TEXT,
    QUESTION_TYPES, FLOW_TYPE_ANY_ANSWER, FLOW_TYPE_SPECIFIC_ANSWER,
    QUESTION_FLOW_TYPES
)
...
class Question(TimeStampedModel):
    ...
    survey = models.ForeignKey(Survey, on_delete=models.CASCADE, related_name='questions', verbose_name=_('опрос'))
    text = models.TextField(_('текст вопроса'))
    question_type = models.CharField(_('тип вопроса'), max_length=20, choices=QUESTION_TYPES, default=QUESTION_TYPE_SINGLE)
...
class QuestionFlow(TimeStampedModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['source_question', 'target_question', 'relationship_type', 'source_answer'],
    ...
    def clean(self):
        if self.relationship_type == FLOW_TYPE_SPECIFIC_ANSWER and self.source_answer is None:
            raise ValidationError(_('Для связи по конкретному ответу необходимо указать ответ'))
            
        if self.source_answer and self.source_answer.question != self.source_question:
            raise ValidationError(_('Выбранный ответ должен принадлежать исходному вопросу'))
        ...
        if self.relationship_type == FLOW_TYPE_SPECIFIC_ANSWER and existing_flows.filter(source_answer=self.source_answer).exists():
